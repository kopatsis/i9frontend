...
export function unravelstretchWO(response) {
	...
	const strRounds = {
		static: {
			samples: workout.StaticSamples,
			titles: workout.StaticNames,
			times: [],
			cycleends: []
		},
		dynamic: {
			samples: workout.DynamicSamples,
			titles: workout.DynamicNames,
			times: [],
			cycleends: []
		}
	};
	...
	for (let h = 0; h < workout.Cycles; h++) {
...
export function unravelWO(response, type = 'Regular') {
	...
	for (let h = 0; h < exerRounds.length; h++) {
		...
		if (workoutRounds.length < 2) {
			...
			workoutRounds[workoutRounds.length - 1].start -=
				workoutRounds[workoutRounds.length - 2].roundrest +
				workoutRounds[workoutRounds.length - 2].off;
		}
	...
	workoutRounds[workoutRounds.length - 1].start -=
		workoutRounds[workoutRounds.length - 2].roundrest + workoutRounds[workoutRounds.length - 2].off;
